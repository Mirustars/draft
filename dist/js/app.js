(() => {
    ...
    let callback = (entries, observer) => {
        entries.forEach((entry => {
            const targetElement = entry.target;
            if (entry.isIntersecting) targetElement.classList.add("show");
        }));
    };
    let observer = new IntersectionObserver(callback, options);
    let itemServicesElements = document.querySelectorAll(".item-services");
    itemServicesElements.forEach((itemServicesElement => {
        observer.observe(itemServicesElement);
    }));
    let itemDestinations = document.querySelectorAll(".item-destinations");
    itemDestinations.forEach((itemDestination => {
        observer.observe(itemDestination);
    }));
    let mainBody = document.querySelector(".main__body");
    observer.observe(mainBody);
    let mediaMain = document.querySelector(".media-main");
    observer.observe(mediaMain);
    let stepsContent = document.querySelector(".steps__content");
    observer.observe(stepsContent);
    let stepsInfoCard = document.querySelector(".steps__info-card");
    observer.observe(stepsInfoCard);
    let testimonialsBody = document.querySelector(".testimonials__body");
    observer.observe(testimonialsBody);
    let clientsItems = document.querySelector(".clients__items");
    observer.observe(clientsItems);
    let subscribeBody = document.querySelector(".subscribe__body");
    observer.observe(subscribeBody);
    window["FLS"] = true;
    isWebp();
    menuInit();
    spollers();
